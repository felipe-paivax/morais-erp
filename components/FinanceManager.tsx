  ...
  return (
    <div className="space-y-8 md:space-y-12 animate-subtle-fade text-white">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-[#222222] pb-6 gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-black tracking-tighter uppercase">Módulo Financeiro</h2>
          <p className="text-xs text-[#F4C150] font-black uppercase tracking-[0.3em] mt-1">Gestão completa de fluxo e custos</p>
        </div>
      </div>

      {/* Tabs Navigation */}
      <div className="flex flex-wrap gap-3 border-b border-[#222222] pb-4">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            ...
            className={`px-6 md:px-8 py-3 md:py-4 rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${
              activeTab === tab.id
                ...
                : 'bg-[#1A1A1A] text-gray-500 border border-[#222222] hover:border-gray-700'
            }`}
          >
            {tab.label}
          </button>
